...
fileName= "/var/www/data/covid19data_"+str(now.year)+"_"+str(now.month)+"_"+str(now.day)+"_"+str(now.hour)+"_"+str(now.minute)+"_"+str(now.second)+".json"
...
for record in data['statewise']:
    ...
    #print(state+","+total+","+active+","+recovered+","+deaths+","+str(sharePercentage))
    #Update each state
...
#json.dump(topo_object,topo_file)
#topo_file.close()
#load the topojson file and update the values



#print(topo_object['objects']['india_state_latest']['geometries'][0]['properties']['NAME_1'])

#save it as different file and update the kml

#Update the time stamp in the website